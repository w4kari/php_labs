...
* **200 OK** — запрос обработан успешно.
  Сервер трактует POST как “выполнить операцию” и возвращает результат без создания нового ресурса.
* **201 Created** — ресурс успешно создан.
  Машина создана как новый ресурс, сервер присвоил id, и может указать Location: /cars/{id}.
* **400 Bad Request** — неверный формат запроса/данных.
  Не хватает обязательного поля, например, нет year.
* **401 Unauthorized** — требуется авторизация, но её нет или она неверна.
  Нужна аутентификация (например, Authorization: Bearer ...), а её нет или токен/логин неверный.
* **403 Forbidden** — авторизация есть, но прав на операцию нет.
  Аутентификация есть, но прав нет, например, пользователь “read-only”, а POST запрещён.
* **404 Not Found** — endpoint не существует на указанном хосте или неверный путь.
  Эндпоинт /cars на этом хосте не существует.
* **500 Internal Server Error** — ошибка на стороне сервера.
  Сервер попытался обработать запрос, но упал из-за бага.
