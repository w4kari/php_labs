...
Для ввода новой записи была разработана форма в файле `index.php`.  
Форма содержит все поля, соответствующие модели данных, использует метод `POST` и отправляет данные на эту же страницу (`index.php`).
...
Обработка формы выполняется в `index.php`.
