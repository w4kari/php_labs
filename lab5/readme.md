...
Строгая типизация нужна для того, чтобы PHP строго проверял типы аргументов и возвращаемых значений. Это помогает находить ошибки раньше, делает код более предсказуемым и понятным, а также снижает вероятность неправильной передачи данных между методами.

### 2. Что такое класс в объектно-ориентированном программировании и какие основные компоненты класса вы знаете?

Класс - это шаблон для создания объектов. Он описывает, какие данные и действия будут у объектов. Основные компоненты класса:
...
Полиморфизм - это возможность работать с разными объектами через общий интерфейс или общий родительский тип. В PHP полиморфизм может быть реализован через интерфейсы и наследование. Например, `TransactionManager` работает не с конкретным `TransactionRepository`, а с `TransactionStorageInterface`. Поэтому можно заменить репозиторий другой реализацией, не меняя код менеджера.

### 4. Что такое интерфейс в PHP и как он отличается от абстрактного класса?

Интерфейс - это набор методов, которые класс обязан реализовать. Интерфейс определяет контракт, но не хранит состояние объекта. Абстрактный класс тоже может задавать обязательные методы, но при этом может содержать уже готовую реализацию, свойства и конструктор.
Главное отличие: интерфейс задает только поведение, а абстрактный класс может частично реализовывать общую логику.
